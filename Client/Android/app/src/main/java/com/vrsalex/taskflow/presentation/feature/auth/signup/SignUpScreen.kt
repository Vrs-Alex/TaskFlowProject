...
fun SignUpScreen(
    ...
    LaunchedEffect(Unit) {
        viewModel.channel.collect {
            when (it) {
                SignUpContract.Effect.OnSignUp -> onSignUp()
                SignUpContract.Effect.OnSignIn -> onSignIn()
            }
        }
    }
    ...
    Column(
        ...
        Column(
            ...
            AppTextInput(
                value = state.email,
                onValueChange = {
                    event(SignUpContract.Event.FieldChanged(SignUpContract.Field.Email, it))
                },
                label = "Email",
            ...
            AppTextInput(
                ...
                onValueChange = {
                    event(SignUpContract.Event.FieldChanged(SignUpContract.Field.Username, it))
                },
                label = "Username",
                placeholder = "PeterK",
            ...
            AppPasswordInput(
                ...
                onValueChange = {
                    event(SignUpContract.Event.FieldChanged(SignUpContract.Field.Password, it))
                },
            ...
            AppPasswordInput(
                ...
                onValueChange = {
                    event(SignUpContract.Event.FieldChanged(SignUpContract.Field.ConfirmPassword, it))
                },
                ...
                keyboardActions = KeyboardActions(
                    onGo = {
                        if (state.isSubmitEnabled) event(SignUpContract.Event.Submit)
                    }
                )
            )
        }
        ...
        Text(
            ...
            modifier = Modifier.align(Alignment.CenterHorizontally).clickable(
                ...
                indication = null,
                onClick = { event(SignUpContract.Event.SignIn) }
            )
        )
        Spacer(modifier = Modifier.height(12.dp))

        AppButton(
            onClick = { event(SignUpContract.Event.Submit) },
