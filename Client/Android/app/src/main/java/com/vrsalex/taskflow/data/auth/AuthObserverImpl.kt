...
class AuthObserverImpl : AuthObserver {
    ...
    private val _logoutObserver = MutableSharedFlow<Unit>(extraBufferCapacity = 1)
    override val logoutObserver: SharedFlow<Unit> = _logoutObserver.asSharedFlow()

    private val _isAuthorized = MutableSharedFlow<Boolean>(replay = 1, extraBufferCapacity = 1)
    override val isAuthorized: SharedFlow<Boolean> = _isAuthorized.asSharedFlow()
